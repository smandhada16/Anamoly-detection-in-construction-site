...
def plot_confusion_matrix_from_data(y_test, predictions, columns=None, annot=True, cmap="Oranges",
    ...
    #show_null_values = 2
    print(columns)
    df1 = DataFrame(confm, index=columns, columns=columns)
    #print(df1)
    ...
    recall = tp / tp_and_fn
    """
    pretty_plot_confusion_matrix(df1, fz=fz, cmap=cmap, figsize=figsize, show_null_values=show_null_values, pred_val_axis=pred_val_axis)
...
def _test_data_class(columns, y_test=[4,5,6,5,4,4,7,8], predic=[4,5,6,5,4,4,7,8]):
    ...
    if(len(y_test) > 10):
        fz=24; figsize=[36,36];
    tp, fp, fn,df1 = plot_confusion_matrix_from_data(y_test, predic, columns,
